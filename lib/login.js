...
const states = [
    {
        ...
        async handler(page, _selected, noPrompt, defaultUsername, defaultPassword) {
    ...
    },
    {
        ...
        async handler(page, selected, noPrompt, defaultUsername, defaultPassword) {
            const error = await page.$(".alert-error");
            if (error) {
                debug("Found error message. Displaying");
                const errorMessage = await page.evaluate(err => err.textContent, error);
...
module.exports = {
    ...
    async _loadProfileAsync(profileName) {
        ...
        for (var prop in env) {
            profile[prop] = env[prop] != null ? env[prop] : profile[prop];
        }
