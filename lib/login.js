...
module.exports = {
    async loginAsync(profileName, mode, disableSandbox, noPrompt, enableChromeNetworkService, awsNoVerifySsl, enableChromeSeamlessSso, additionalStateFile) {
        ...
        if ( (!additionalStateFile) && profile.azure_custom_login_statefile ) {
            additionalStateFile = profile.azure_custom_login_statefile;
        }
        const additionalStates = this._loadAdditionalStates(additionalStateFile);
        const states = [].concat(defaultStates).concat(additionalStates);
        const loginUrl = await this._createLoginUrlAsync(profile.azure_app_id_uri, profile.azure_tenant_id);
        const samlResponse = await this._performLoginAsync(loginUrl, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, profile.azure_default_username, profile.azure_default_password, enableChromeSeamlessSso, states);
    ...
    _loadAdditionalStates(stateFile) {
        if ( stateFile === undefined || stateFile === null ) {
            return [];
        }
        try {
            return eval( fs.readFileSync(stateFile).toString() );
        } catch(e) {
            console.log(e);
            throw new CLIError(`Unable to read additional state file: ${stateFile}`);
        }
        return [];
    },
     ...
     * @private
     */
    async _performLoginAsync(url, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, defaultUsername, defaultPassword, enableChromeSeamlessSso, states) {
