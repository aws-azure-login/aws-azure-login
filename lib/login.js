...
module.exports = {
    async loginAsync(profileName, mode, disableSandbox, noPrompt, enableChromeNetworkService, awsNoVerifySsl, enableChromeSeamlessSso, authWhitelist) {
        ...
        const samlResponse = await this._performLoginAsync(loginUrl, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, profile.azure_default_username, profile.azure_default_password, enableChromeSeamlessSso, authWhitelist);
     ...
     * @param {bool} [enableChromeSeamlessSso] - Enable chrome pass-through authentication with AAD seamless SSO
     * @param {Array.<string>} [authWhitelist] - SPNEGO whitelist next to the default AAD SSO domain
     * @returns {Promise.<string>} The SAML response.
     * @private
     */
    async _performLoginAsync(url, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, defaultUsername, defaultPassword, enableChromeSeamlessSso, authWhitelist) {
        ...
        try {
            ...
            const authWhitelistParam = authWhitelist.concat(AZURE_AD_SSO).join(',');
            if (enableChromeSeamlessSso) args.push(
                `--auth-server-whitelist=${authWhitelistParam}`,
                `--auth-negotiate-delegate-whitelist=${authWhitelistParam}`
            );
