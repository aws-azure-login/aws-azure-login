...
module.exports = {
    async loginAsync(profileName, mode) {
        ...
        const samlResponse = await this._performLoginAsync(loginUrl, headless, cliProxy, profile.azure_default_username);
     ...
     * @private
     */
    async _performLoginAsync(url, headless, cliProxy, defaultUsername) {
                ...
                while (true) {
                    ...
                    if (foundState) {
                        ...
                        if (totalUnrecognizedDelay > MAX_UNRECOGNIZED_PAGE_DELAY) {
                            throw new CLIError("Unable to determine login page delay. If this problem persists, try running with --mode=gui or --mode=debug");
                        }
